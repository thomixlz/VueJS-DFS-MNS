<template>
  <div class="container">
    <h1 class="my-4">Ajouter une facture</h1>
...
export default {
  setup() {
    ...
    const facture = reactive({
      ...
      client: null,
      prestations: [
        { id: Date.now(), description: '', quantite: 0, montantUnitaire: 0, montantTotal: 0 }
      ],
      remises: '0,00 €',
      dejaPaye: '0,00 €',
    });
    ...
    const addFacture = () => {
      facturesStore.addFacture({ ...facture, id: Date.now() });
      router.push('/factures');
    };

    const addPrestation = () => {
      facture.prestations.push({ id: Date.now() + facture.prestations.length, description: '', quantite: 0, montantUnitaire: 0, montantTotal: 0 });
    };
